...
class VGG_A_BatchNorm(nn.Module):
    """VGG_A with BatchNorm after each conv (Conv -> BN -> ReLU -> Pool)."""

    def __init__(self, inp_ch=3, num_classes=10, init_weights=True):
        super().__init__()

        self.features = nn.Sequential(
            _conv_bn_relu(inp_ch, 64),
            nn.MaxPool2d(kernel_size=2, stride=2),
...
def build_model(model_name: str, num_classes: int = 10):
    if model_name == "VGG_A":
        return VGG_A(num_classes=num_classes)
    if model_name == "VGG_A_BatchNorm":
        return VGG_A_BatchNorm(num_classes=num_classes)
